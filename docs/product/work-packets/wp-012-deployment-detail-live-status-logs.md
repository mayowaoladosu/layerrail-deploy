...
Authenticated users can open an organization from the Rails home page, filter its latest Deployment history by status, and inspect one Deployment. The detail view presents status, environment, source reference/digest, initiating actor, timestamps, execution duration, trigger, local resource size, immutable and environment URLs, current/previous Alias pointers, structured failure context, Build/runtime/system logs, and the complete persisted transition timeline.
...
- The complete control-plane CI passes 267 examples, RuboCop, Zeitwerk, Bundler Audit, Importmap Audit, and Brakeman.
