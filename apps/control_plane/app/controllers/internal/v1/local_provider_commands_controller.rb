module Internal
  module V1
    class LocalProviderCommandsController < BaseController
      ...
      def finalize
        event = OutboxEvent.find_by(id: params[:event_id])
        return render json: { code: "not_found", message: "Command was not found" }, status: :not_found unless event
      ...
      def render_stale_claim
        render json: { code: "stale_claim", message: "Command lease is no longer active" }, status: :conflict
      end

      def render_outcome_conflict
        render json: { code: "outcome_conflict", message: "Command already has a different outcome" }, status: :conflict
      end
    end
  end
end
