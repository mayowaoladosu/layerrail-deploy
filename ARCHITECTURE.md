...
Rodauth Rails is the control plane's sole authentication framework. Passwordless email links, configured GitHub/Google login, encrypted browser cookies, JWT Bearer sessions and PostgreSQL-backed revocation all enter through Rodauth. API routes never accept browser-cookie fallback. Application code adds digest-only request throttling, concurrent one-time-link claims and first-owner election around that boundary; Pundit memberships remain responsible for organization authorization. FastAPI and Rails both use Resend unless complete SMTP settings explicitly override it; credential-free development writes email to disk. The Rails authentication pages intentionally reproduce the legacy FastAPI auth shell while the two applications run side by side.

The Devpush Jinja dashboard and its compiled Basecoat assets are the presentation contract for the rebuild. Rails translates each owned page to ERB while retaining the team/project/Deployment route hierarchy, header switchers, tabs, filters, menus, empty states and responsive composition. Turbo Streams and Stimulus may provide the live implementation, but new durable state and Phase 2 behavior must appear inside this dashboard rather than a parallel control-plane UI. Rails reads the canonical assets and icons from a read-only mount; it does not import Python code or share the FastAPI schema.
...
WP-012 adds a signed, tenant-bound runtime-log read contract. The provider reads only the validated managed container, removes common credential patterns and internal health-probe noise, bounds each response, and retains the latest redacted snapshot in SQLite before cancellation removes the container. Rails merges that output with persisted Deployment transitions and Build lifecycle entries; provider outages degrade to persisted history rather than erasing status.
